...
| # | Tarea | Estado | Notas |
|---|-------|--------|-------|
| 1 | **Deploy fix cooldown admin** | ✅ Código listo, pendiente deploy | `handler.py` — cooldown per-chat usando `remote_jid` normalizado. Fix del bug donde el bot respondía cuando el admin escribía primero. |
| 2 | **Mensajes interactivos (botones/menús)** | 🔲 Sin empezar | El adapter actual solo envía texto. Necesita `sendButtons`, `sendList`, `sendCTA` en `evolution/adapter.py`. |
| 3 | **Flujo de citas (Nivel 1)** | 🔲 Sin empezar | Prompt del bot + lógica de recolección fecha/hora + notificación al admin por WhatsApp. |
| 4 | **Catálogo de servicios por tenant** | 🔲 Sin empezar | El bot necesita saber QUÉ se agenda, a qué precio, y duración. Modelar en `business_config`. |
...
2. Mensajes interactivos (#2) — da cara profesional
3. Flujo de citas (#3) — permite cobrar
4. Catálogo de servicios (#4) — el bot necesita saber qué vende
