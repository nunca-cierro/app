...
## 🤖 FASE 5 — Prompt del Bot: Fuente Única de Verdad
...
| 22 | **Eliminar custom prompt y first_message_hint** | ✅ Desplegado | business_config es la ÚNICA fuente de verdad. Eliminados fallbacks innecesarios. Logs warning claro si agent no tiene business_config. Commit `91b5629`. |
| 23 | **Prioridad clara de instrucciones** | ✅ Resuelto | business_config > emergency fallback (solo si no hay agent). Sin más capas compitiendo. |
